...
| `.imageSupersampling(_:)`, `.imageEdgeThreshold(_:)` | image-fidelity knobs: source-pixels-per-cell averaging for the luminance-ramp renderers (1...4; nil = default), and the shape-aware ascii/unicode edge-glyph gradient threshold (lower = more edges; nil = pure coverage matching) |
