...
|---|---|---|
| **Drag-to-reorder rows** in `List`/`Table` | drag a row (edit mode / backed by `.onMove`) to a new slot | **shipped** (List + `ForEach.onMove`): mouse-press picks a row up (highlighted via the focus cursor), drag moves the cue to the row under the pointer, drop commits one `onMove(source, destination)`. Commit-on-release keeps the list order — and the captured drag geometry — stable through the gesture. Follow-ups: a between-rows insertion indicator / live continuous shuffle, and `Table` parity. |
