...
| Controls | `Button` (string **and** `Button(action:label:)`), `Toggle`, `Slider`, `Stepper`, `ProgressView`, `Gauge`, `TextField`, `SecureField`, `TextEditor`, `Picker`, `DatePicker`, `Link`, `ColorPicker`, `Divider`, `Spacer`, `EmptyView`, `AnyView`, `ContentUnavailableView` | ✓ | `Slider`/`ProgressView`/`Stepper`/`Gauge` are floating-point-capable; custom `ButtonStyle`/`ToggleStyle` via `makeBody`; `PickerStyle` is a marker protocol, matching SwiftUI; `Link` opens via `@Environment(\.openURL)` (no OSC 8 — §2.4-style deviation); `TextEditor` scrolls to follow the cursor (no soft-wrap yet); `DatePicker` is an inline numeric field (no calendar popup, fixed numeric format); multi-selection is `List(selection: Binding<Set<…>>)` (SwiftUI has no multi-select `Picker`) |
