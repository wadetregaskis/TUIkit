...
| **Common modifiers: `.opacity`(View), `.truncationMode`(View), `.onSubmit`/`.submitLabel`, `.searchable`, `.refreshable`, `.contextMenu`, `.onReceive`** | Frequently used; each terminal-expressible. | `.id(_:)` **shipped** (splices a keyed identity step — changing the id resets the subtree's `@State`, re-fires `onAppear`/`task`, and discards its cached buffer). `.focusable(_:interactions:)` **shipped** (makes any view a Tab stop, plus click-to-focus for `.activate`; it's what lets `.focused($x)` bind to a plain view like `Text`). `.searchable` is the next highest-value. `.opacity` → dim/blend approximation only. (`.multilineTextAlignment` shipped — §1.) |
