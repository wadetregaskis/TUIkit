...
| **Common modifiers: `.opacity`(View), `.truncationMode`(View), `.onSubmit`/`.submitLabel`, `.focusable`, `.searchable`, `.refreshable`, `.contextMenu`, `.onReceive`** | Frequently used; each terminal-expressible. | `.id(_:)` **shipped** (splices a keyed identity step — changing the id resets the subtree's `@State`, re-fires `onAppear`/`task`, and discards its cached buffer). `.searchable` is the next highest-value. `.opacity` → dim/blend approximation only. (`.multilineTextAlignment` shipped — §1.) |
