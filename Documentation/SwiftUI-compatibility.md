...
| **List editing: `onDelete`/`onMove`, `.listRowInsets`/`.listRowBackground`/`.listSectionSeparator`** | Editable lists. | `EditMode` + `\.editMode` + `EditButton` **shipped** (the edit-mode scaffolding: a `Sendable`/`Equatable` `EditMode`, a nil-default optional `\.editMode` binding, and an `EditButton` that toggles it and disables itself when no binding is in scope). Remaining: wire `onMove`/`onDelete` into the row model (key-driven move/delete) — `onMove` is also the API *behind* the implicit drag-to-reorder behaviour (see the behaviours table below), reusing the shipped Transferable drag infra + the gradient-stop live-reorder pattern for a nearest-slot row drag. `.listRow*` modifiers TBD. |
