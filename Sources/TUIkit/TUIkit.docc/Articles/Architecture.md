...
Rendering is **demand-driven and frame-capped**, not a fixed-rate poll. With nothing pending and nothing animating, the loop blocks in `await stdinArrival.waitForArrival(...)` until woken — by terminal input, a render request, or a signal — so a static screen does zero renders and consumes no CPU. When work is pending it renders at most once per frame interval (`App.maxFrameRate`, default 60 FPS), coalescing a burst of requests into a single frame. Each iteration checks `shouldShutdown` (set by SIGINT or SIGTERM) and the in-app dismiss flag, consumes the resize flag to invalidate the diff cache if SIGWINCH fired, drains up to 128 pending terminal events (keys and mouse) and dispatches each — keys through five handler layers, mouse through the hit-test dispatcher — then renders if a frame is due and the frame-rate cap has cleared. The wait is a real suspension point, so it releases the main actor and lets `Task`, `MainActor.run`, and `DispatchQueue.main` work run between frames. Signals arrive as ordinary libdispatch events: `SignalManager` monitors SIGWINCH (resize) and SIGINT/SIGTERM (graceful shutdown) with `DispatchSource.makeSignalSource` on the main queue, and each source's handler sets its flag and `wake()`s the idle-blocked loop. Other asynchronous render triggers (animation deadlines, @State changes, focus changes) feed back into the render decision via `appState.needsRender`, which also `wake()`s the loop.

@Image(source: "architecture-event-loop.svg", alt: "Flowchart of the TUIkit demand-driven event loop: after subsystems are initialised the loop checks shouldShutdown; if not shutting down it consumes the resize flag (invalidating the diff cache on SIGWINCH), drains up to 128 key and mouse events per frame and dispatches them (keys through five layers plus the ESC pre-route and semantic-shortcut stages, mouse through hit-testing), renders at most once per App.maxFrameRate when a frame is due, then blocks until woken by input, a render request, or a signal before looping again. When shouldShutdown is true it cleans up and exits.")
