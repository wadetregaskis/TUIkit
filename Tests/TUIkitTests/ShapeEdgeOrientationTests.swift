...
//
//  The shape renderer now detects a strong directional edge in each cell (via a
//  Sobel-style gradient over its six sampling regions) and draws it with the
//  orientation-matched line glyph; `.shapeUnicode` uses box-drawing lines.
//
...
struct ShapeEdgeOrientationTests {
    ...
    private func render(_ img: RGBAImage, w: Int, h: Int, _ set: ASCIICharacterSet) -> String {
        ASCIIConverter(characterSet: set, colorMode: .mono, dithering: .none)
    ...
    func verticalEdge() {
        ...
        let ascii = render(img, w: 20, h: 3, .shapeBased)
        #expect(ascii.contains("|"), "a vertical edge picks '|': \(ascii)")

        let unicode = render(img, w: 20, h: 3, .shapeUnicode)
        #expect(unicode.contains("│"), "shapeUnicode uses '│' for a vertical edge: \(unicode)")
        #expect(!unicode.contains("|"), "shapeUnicode uses box-drawing, not ASCII '|'")
    }
    ...
    func horizontalEdge() {
        ...
        let ascii = render(img, w: 18, h: 6, .shapeBased)
        #expect(ascii.contains("-"), "a horizontal edge picks '-': \(ascii)")

        let unicode = render(img, w: 18, h: 6, .shapeUnicode)
        #expect(unicode.contains("─"), "shapeUnicode uses '─' for a horizontal edge: \(unicode)")
    }
    ...
    func diagonalEdge() {
        ...
        let ascii = render(img, w: 18, h: 9, .shapeBased)
        #expect(ascii.contains("/") || ascii.contains("\\"), "a diagonal edge picks a slash: \(ascii)")

        let unicode = render(img, w: 18, h: 9, .shapeUnicode)
        #expect(
            unicode.contains("╱") || unicode.contains("╲"),
            "shapeUnicode uses box-drawing diagonals: \(unicode)")
    }
    ...
    func flatCellNoEdge() {
        ...
        let ascii = render(img, w: 18, h: 3, .shapeBased)
