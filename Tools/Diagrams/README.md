...
Every diagram in the DocC catalog is generated by this tool. The only remaining
`Resources/*.png` files are the TUIkit logos, which aren't diagrams.

The model handles the shapes these diagrams need: top-to-bottom or left-to-right
flow (`rankdir`), decision diamonds, a loop-back edge (`loop: true`), dashed
"uses" edges (`dashed: true`), and a grouping box around a set of nodes
(`clusters`). Graphviz lays out everything — including the dependency-graph DAGs —
so adding or changing a diagram is just editing its `Diagram` value.
